...
@Component({
  selector: 'vex-shippingcountryedit-model',
  templateUrl: './shippingcountryedit-model.component.html',
  styleUrls: ['./shippingcountryedit-model.component.scss'],
...
export class ShippingcountryeditModelComponent implements OnInit {
  ...
  columns: TableColumn<Createorder>[] = [
    ...
    { label: 'Customer Name', property: 'cust_name', type: 'text', visible: true },
    { label: 'Customer Email', property: 'cust_email', type: 'text', visible: true },
  ...
  constructor(public dialogRef: MatDialogRef<ShippingcountryeditModelComponent>,@Inject(MAT_DIALOG_DATA) public data: any, private cd: ChangeDetectorRef, private urlApi: UrlAPIService) { }
  ...
  setCountry(e){
    debugger;
    if(e.target.checked){
      this.countries.push(e.target.value);
    }
    else{
      const index = this.countries.indexOf(e.target.value);
  ...
  save(){
    this.data.onCountrysave(this.countries)
  }
}
