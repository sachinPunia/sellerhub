...
@Component({
  selector: 'vex-ebaygeneratetoken-model',
  templateUrl: './ebaygeneratetoken-model.component.html',
  styleUrls: ['./ebaygeneratetoken-model.component.scss']
})
export class EbaygeneratetokenModelComponent implements OnInit {
  ...
  generatetoken(){
    this.urlApi.getEbayToken(this.id).subscribe(() => {},(token:any) => {
      // alert(token);
      // debugger;
      console.log(token);
      this.result = token;
      ...
      // w.location.assign(token.error.text); 
      (window as any).open(token.error.text, "_blank");
      // w.open(token.error.text, "_blank");
